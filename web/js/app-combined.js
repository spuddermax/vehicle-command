...
function BatteryGauge({ batteryLevel, range = 250 }) {
	...
	const batteryColor =
		percentage > 50 ? "#00ff88" : percentage > 20 ? "#ffaa00" : "#ff4444";
...
function TemperatureGauge({ temperature, label = "MOTOR" }) {
	const tempColor =
		temperature > 80 ? "#ff4444" : temperature > 60 ? "#ffaa00" : "#00ff88";
...
function Dashboard({ dashboardState }) {
	return (
		<GridWithOverlay>
			...
			<BatteryGauge
				batteryLevel={dashboardState.batteryLevel}
				range={dashboardState.range}
			/>
			<PowerMeter power={dashboardState.power} />
			<TemperatureGauge
				temperature={dashboardState.motorTemp}
				label="MOTOR"
			/>
			<TemperatureGauge
				temperature={dashboardState.batteryTemp}
				label="BATTERY"
			/>
		</GridWithOverlay>
	);
			...
			<div className="temperature-display">
				<div className="temp-side">
					...
					<div className="temp-control-row">
						<button
							className="temp-button temp-minus"
							onClick={() => handleTempChange("driver", -1)}
							disabled={disabled || driverTemp <= 60}>
							−
						</button>
						<div className="temp-value">
							{Math.round(driverTemp)}°F
						</div>
						<button
							className="temp-button temp-plus"
							onClick={() => handleTempChange("driver", 1)}
							disabled={disabled || driverTemp >= 85}>
							+
						</button>
					</div>
					<div className="temp-slider-container">
						<input
							...
							onChange={(e) =>
								handleTempChange(
									"driver",
									parseFloat(e.target.value) - driverTemp
								)
							}
							disabled={disabled}
						...
						<div className="temp-labels">
							<span
								onClick={() =>
									handleTempChange("driver", 60 - driverTemp)
								}>
								60
							</span>
							<span
								onClick={() =>
									handleTempChange("driver", 65 - driverTemp)
								}>
								65
							</span>
							<span
								onClick={() =>
									handleTempChange("driver", 70 - driverTemp)
								}>
								70
							</span>
							<span
								onClick={() =>
									handleTempChange("driver", 75 - driverTemp)
								}>
								75
							</span>
							<span
								onClick={() =>
									handleTempChange("driver", 80 - driverTemp)
								}>
								80
							</span>
							<span
								onClick={() =>
									handleTempChange("driver", 85 - driverTemp)
								}>
								85
							</span>
						</div>
				...
				<div className="temp-side">
					...
					<div className="temp-control-row">
						<button
							...
							disabled={disabled || passengerTemp <= 60}>
							−
						</button>
						<div className="temp-value">
							{Math.round(passengerTemp)}°F
						</div>
						<button
							className="temp-button temp-plus"
							onClick={() => handleTempChange("passenger", 1)}
							disabled={disabled || passengerTemp >= 85}>
							+
						</button>
					</div>
					<div className="temp-slider-container">
						<input
							...
							onChange={(e) =>
								handleTempChange(
									"passenger",
									parseFloat(e.target.value) - passengerTemp
								)
							}
							disabled={disabled}
							className="temp-slider"
...
function AirflowControl({
	...
	// Parse current pattern to determine individual states
	const getIndividualStates = (pattern) => {
		if (pattern === "feet-face")
			return { face: true, feet: true, defrost: false };
		if (pattern === "feet-defrost")
			return { face: false, feet: true, defrost: true };
		if (pattern === "face-defrost")
			return { face: true, feet: false, defrost: true };
		if (pattern === "face-feet-defrost")
			return { face: true, feet: true, defrost: true };
		if (pattern === "auto")
			return { face: false, feet: false, defrost: false };
		if (pattern === "face")
			return { face: true, feet: false, defrost: false };
		if (pattern === "feet")
			return { face: false, feet: true, defrost: false };
		if (pattern === "defrost")
			return { face: false, feet: false, defrost: true };
		return { face: false, feet: false, defrost: false };
	};

	const currentStates = getIndividualStates(airflowPattern);
	...
	const handleIndividualToggle = (type) => {
		...
		// Check if this would turn off the last active button
		const activeCount = Object.values(newStates).filter(Boolean).length;
		if (activeCount === 0) {
			// Don't allow turning off the last active button
			return;
		}

		// Determine the new pattern based on individual states
		let newPattern;
		if (newStates.face && newStates.feet && newStates.defrost) {
			newPattern = "face-feet-defrost"; // All three on
		} else if (newStates.face && newStates.feet && !newStates.defrost) {
			newPattern = "feet-face";
		} else if (!newStates.face && newStates.feet && newStates.defrost) {
			newPattern = "feet-defrost";
		} else if (newStates.face && !newStates.feet && newStates.defrost) {
			newPattern = "face-defrost"; // Face + defrost combination
		} else if (newStates.face && !newStates.feet && !newStates.defrost) {
			newPattern = "face";
		} else if (!newStates.face && newStates.feet && !newStates.defrost) {
			newPattern = "feet";
			...
			newPattern = "face"; // Default fallback
		}

		onAirflowChange(newPattern);
	};

	const handleCombinationClick = (pattern) => {
		if (disabled) return;
		onAirflowChange(pattern);
	};

	return (
		<Card title="AIRFLOW" className="airflow-control">
			<div className="airflow-grid-with-auto">
				{/* AUTO button - spans 2 rows */}
				<button
					className={`airflow-button airflow-auto ${
						airflowPattern === "auto" ? "active" : ""
					}`}
					onClick={() => handleCombinationClick("auto")}
					disabled={disabled}>
					<div className="airflow-icon">🔄</div>
					<div className="airflow-label">AUTO</div>
				</button>

				{/* Individual toggle buttons */}
				<button
					className={`airflow-button airflow-individual ${
						currentStates.feet ? "active" : ""
					}`}
					onClick={() => handleIndividualToggle("feet")}
					disabled={disabled}>
					<div className="airflow-icon">🦶</div>
					<div className="airflow-label">FEET</div>
				</button>

				<button
					className={`airflow-button airflow-individual ${
						currentStates.face ? "active" : ""
					}`}
					onClick={() => handleIndividualToggle("face")}
					disabled={disabled}>
					<div className="airflow-icon">👤</div>
					<div className="airflow-label">FACE</div>
				</button>

				<button
					className={`airflow-button airflow-individual ${
						currentStates.defrost ? "active" : ""
					}`}
					onClick={() => handleIndividualToggle("defrost")}
					disabled={disabled}>
					<div className="airflow-icon">❄️</div>
					<div className="airflow-label">DEFROST</div>
				</button>

				{/* Combination buttons */}
				<button
					className={`airflow-button airflow-combination ${
						airflowPattern === "feet-face" ? "active" : ""
					}`}
					onClick={() => handleCombinationClick("feet-face")}
					disabled={disabled}>
					<div className="airflow-icon">🦶👤</div>
					<div className="airflow-label">FEET+FACE</div>
				</button>

				<button
					className={`airflow-button airflow-combination ${
						airflowPattern === "feet-defrost" ? "active" : ""
					}`}
					onClick={() => handleCombinationClick("feet-defrost")}
					disabled={disabled}>
					<div className="airflow-icon">🦶❄️</div>
					<div className="airflow-label">FEET+DEFROST</div>
				</button>

				<button
					className={`airflow-button airflow-combination ${
						airflowPattern === "face-defrost" ? "active" : ""
					}`}
					onClick={() => handleCombinationClick("face-defrost")}
					disabled={disabled}>
					<div className="airflow-icon">👤❄️</div>
					<div className="airflow-label">FACE+DEFROST</div>
				</button>

				{/* Recirculate button - spans 2 rows */}
				<button
					className={`airflow-button airflow-recirculate ${
						recirculateOn ? "active" : ""
					}`}
					onClick={() => onRecirculateChange(!recirculateOn)}
					disabled={disabled}>
					<div className="airflow-icon">🔄</div>
					<div className="airflow-label">RECIRC</div>
				</button>
			</div>
		</Card>
	);
			...
			<div className="status-grid">
				<div className="status-item">
					...
					<div className="status-value">
						{Math.round(hvacState.insideTemp)}°F
					</div>
				</div>
				<div className="status-item">
					<div className="status-label">OUTSIDE</div>
					<div className="status-value">
						{Math.round(hvacState.outsideTemp)}°F
					</div>
				</div>
				<div className="status-item">
...
function TeslaHVACApp() {
	...
	const setTemperature = useCallback(
		async (driverTemp, passengerTemp) => {
	...
	const setAirflowPattern = useCallback(
		async (pattern) => {
			if (connectionStatus !== "connected") return;
	...
	const setRecirculate = useCallback(
		async (recirculateOn) => {
			if (connectionStatus !== "connected") return;
			...
			try {
				...
				setError("Failed to set recirculate");
			}
		},
		[connectionStatus]
	);
	...
	const renderTabContent = () => {
		if (activeTab === "instruments") {
			return <Dashboard dashboardState={dashboardState} />;
				...
				<>
					<GridWithOverlay>
						...
						<AirflowControl
							...
							recirculateOn={hvacState.recirculateOn}
							onAirflowChange={setAirflowPattern}
							onRecirculateChange={setRecirculate}
							disabled={!hvacState.isOn}
						/>
